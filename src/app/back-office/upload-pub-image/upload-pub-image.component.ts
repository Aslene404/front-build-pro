...
@Component({
  selector: 'app-upload-pub-image',
  templateUrl: './upload-pub-image.component.html',
  styleUrls: ['./upload-pub-image.component.css']
})
export class UploadPubImageComponent implements OnInit {
  @Input() pubId: string;
  ...
  constructor(private pubService: PubsService,private snackBar: MatSnackBar) {
  ...
  startUpload(): void {
    const event: UploadInput = {
      type: 'uploadAll',
      url: `${environment.API_URL}/api/v1/upload/pub/image`,
      method: 'POST',
      data: { id: this.pubId }
    };
